...
int main(int argc, char *argv[]) {
  ...
  read_bmp("lena_std_short_noise.bmp");
  printf("======================================\n");
      ...
      for(int v = -1; v <= 1; v ++){
        for(int u = -1; u <= 1; u++){
          if(x + u >= 0 && x + u < width && y + v >= 0 && y + v < height){
            buffer[0] = *(source_bitmap + bytes_per_pixel * (width * (y + v) + (x + u)) + 2);
            buffer[1] = *(source_bitmap + bytes_per_pixel * (width * (y + v) + (x + u)) + 1);
            buffer[2] = *(source_bitmap + bytes_per_pixel * (width * (y + v) + (x + u)) + 0);
            buffer[3] = 0;
          }else{
            buffer[0] = 0;
            buffer[1] = 0;
